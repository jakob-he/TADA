...
class Gene(Bed):
    ...
    def __str__(self):
        """SV Objects are representated by all the columns entered in the bed file"""
    ...
    def get_exon_overlap(self,sv):
        """Return proportion of exons overlapping with the given sv"""
        if self.annotations['EXONS']:
            exon_prop = sum([1 if utils.getOverlap((sv.start,sv.end),(exon.start,exon.end)) > 0 else 0 for exon in self.annotations['EXONS']])/len(self.annotations['EXONS'])
            return exon_prop
        else:
            return 0

    def get_interaction_overlap(self,sv):
        """Return proportion of interacting fragments overlapping with the given sv normalized by the LOEUF score"""
        if self.annotations['fragments']:
            fragment_prop = sum([1 if utils.getOverlap((sv.start,sv.end),(fragment[1],fragment[2])) > 0 else 0 for fragment in self.annotations['fragments']])/len(self.annotations['fragments'])
