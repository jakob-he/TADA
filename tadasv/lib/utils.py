...
def objects_from_file(path, cls_string, column_names=[], **kwargs):
    ...
    for line in open_path:
        ...
        # handle headers of bed SV files
        if line.startswith('##'):
            vcf = True
            continue
        if line.startswith('#'):
...
def create_chr_dictionary_from_beds(beds):
...
def annotate_svs(tad_dict, sv_dict):
    """Finds all TADs overlapping with the SV, then constructs a new chrom dictionary with the annotated SVs.
    The function iterates through the TADs one chromsome at a time. For each TAD it checks every SVs were
    the start position is either less or equal to the TADs start position. If that is the case there are four possiblities:
        1. The SV ends before the TAD -> this SV is either not in any of the available TADs or ended in between TADs.
        2. The SV ends in the TAD but starts before it -> append the TAD to the SV and move the SV to the list of annotated SVs.
        3. The SV ends and starts in the TAD -> append the TAD to the SV and move the SV to the list of annotated SVs.
        4. Tne SVs starts before the TAD but ends after it -> append the TAD to the SV, keep it in the SV dict.
    """
    # reduce the svs to chromsomes were tads are available
    sv_dict = reduce_dict(sv_dict, tad_dict.keys())
    # create empty list
    annotated_svs = []

    # iterate through SVs one chromsome at a time
    for chrom in sv_dict:
        for tad in tad_dict[chrom]:
            sv_queue = []
            while is_in(sv_dict[chrom], tad):
                if sv_dict[chrom][0].end <= tad.end and sv_dict[chrom][0].end >= tad.start:
                    sv_dict[chrom][0].tads.append(tad)
                    annotated_svs.append(sv_dict[chrom].pop(0))
                elif sv_dict[chrom][0].end > tad.end:
                    sv_dict[chrom][0].tads.append(tad)
                    sv_queue.append(sv_dict[chrom].pop(0))
                else:
                    annotated_svs.append(sv_dict[chrom].pop(0))
            sv_dict[chrom] = sv_queue + sv_dict[chrom]
        annotated_svs.extend(sv_dict[chrom])
    return create_chr_dictionary_from_beds(annotated_svs)
