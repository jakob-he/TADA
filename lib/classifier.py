...
class Classifier():
    ...
    def train(self, train_set, output_dir='./', permut_importance=True, gridcv=False):
        ...
        if gridcv:
            ...
            gscv = GridSearchCV(
                self.pipeline, gridcv_parameters_dict[self.name], cv=skf, verbose=1, scoring="roc_auc")
            # Fit gscv
        ...
        if permut_importance:
            ...
            sorted_scores, sorted_clusters = list(zip(
                *sorted([(round(score, 4), cluster) for cluster, score in scores.items()], reverse=False)))
